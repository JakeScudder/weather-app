...
const Nav = (props) => {
  ...
  const [nav1, setNav1] = useState(
    Cookies.getJSON("nav1") || "Charlottesville, VA"
  );
  ...
  const [nav3, setNav3] = useState(
    Cookies.getJSON("nav3") || "Jacksonville, FL"
  );
  ...
  const showHideInput = () => {
    ...
    //Show Input area if an option is selected
  ...
  // When user is typing fetch Autofill data and fill array
  const handleChange = (event) => {
    setInput(event.target.value);
    axios
      .get(
        `https://cors-anywhere.herokuapp.com/https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${event.target.value}&key=${googleKey}&sessiontoken=${unique}`
      )
      .then((response) => {
        ...
        setAutofill(response.data.predictions);
      });
  };

  // This function sets the url links when a location is manually entered by keyboard
  ...
  // This function sets the url links once an autofill selection is clicked
  ...
  return (
    <div id="nav-container">
      <div id="mainNav">
        <ul>
          <li>
            <NavLink exact to="link1" name={nav1} onClick={setLink}>
              {url1}
            </NavLink>
          </li>
          <li>
            <NavLink to="link2" name={nav2} onClick={setLink}>
              {url2}
            </NavLink>
          </li>
          <li>
            <NavLink to="link3" name={nav3} onClick={setLink}>
              {url3}
            </NavLink>
          </li>
        </ul>
      ...
      <div id="form-div">
        ...
        >
          ...
          <ul
            id="autofill-changeLink"
            style={{ display: showingAuto ? "block" : "none" }}
          >
            {showingAuto && autofill.length > 1
              ? autofill.slice(0, 4).map((suggestion) => {
                  ...
                  return (
                    <li
                      key={suggestion.id}
                      className="autofill-changeLink-li"
